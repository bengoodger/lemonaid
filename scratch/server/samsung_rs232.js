...
http.createServer(function(request, response) {
  ...
  if (request.url.endsWith("/tvOn")) {
    console.log("TV On");
    var client = net.connect(4998, "10.1.10.51", function() {
      client.write(new Buffer([0x08, 0x22, 0x00, 0x00, 0x00, 0x02, 0xd6, 0x0d]));
    });
  } else if (request.url.endsWith("/tvOff")) {
    console.log("TV Off");
    var client = net.connect(4998, "10.1.10.51", function() {
      client.write(new Buffer([0x08, 0x22, 0x00, 0x00, 0x00, 0x01, 0xd5, 0x0d]));
    });
